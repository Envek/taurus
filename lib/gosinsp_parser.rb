      ...
      plan.css("Документ>План>СтрокиПлана>Строка").each do |row|
        ...
        if discipline
          ...
          if row["Кафедра"] and dept
            department_ids = Discipline.where(name: row["Дис"]).joins(:department).pluck(:department_id)
            departments = Department.where(id: department_ids)
            errors << "Дисциплина «#{row["Дис"]}» не найдена на кафедре #{dept.name}."
            errors[-1] += " Такая дисциплина найдена на кафедрах: #{departments.pluck(:name).join(', ')}" if departments.any?
          else
