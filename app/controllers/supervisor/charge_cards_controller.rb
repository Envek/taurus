...
class Supervisor::ChargeCardsController < ApplicationController
  active_scaffold :charge_card do |conf|
    conf.actions << :delete
    conf.columns = [:teaching_place, :assistant_teaching_place, :lesson_type, :jets, :disciplines, :hours_quantity, :hours_per_week, :weeks_quantity, :groups, :preferred_classrooms, :note]
