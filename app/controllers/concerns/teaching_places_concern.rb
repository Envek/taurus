...
module TeachingPlacesConcern
  ...
  def training_assignments
    cc_includes = [:disciplines, {jets: :group}]
    ...
    disciplines = (@teaching_place.charge_cards.map(&:disciplines) + @teaching_place.assistant_charge_cards.map(&:disciplines)).flatten.compact.uniq.sort_by(&:name)
    @grouped = []
    disciplines.each do |discipline|
      cc = @charge_cards.select{|cc| cc.discipline_ids.include? discipline.id}
